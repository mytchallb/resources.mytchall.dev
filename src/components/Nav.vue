    ...
    <div class="flex gap-4 justify-between items-center">
      ...
      <div class="flex gap-1 flex-1">
        <span
          v-for="tag in tags"
          :key="tag"
          class="capitalize bg-primary text-copy px-2 py-2 rounded-md cursor-pointer hover:bg-accent"
          >{{ tag }}</span
        >
      </div>
      <select
        class="bg-background text-copy border-2 border-primary rounded-md px-2 py-1"
      >
